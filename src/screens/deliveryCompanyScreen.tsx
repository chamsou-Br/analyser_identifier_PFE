...
import {
  addDeliveryCompanyAPI,
  deleteDeliveryCompanyAPI,
  fetchDeliveryCompanyAPI,
...
const DeliveryCompanyScreen: React.FC = () => {
  ...
  const [isDeleteCompanyConfirmation, setIsDeleteCompanyConfirmation] =
    useState(false);
  ...
  const onConfirmAddCompany = async (
    company: string,
    email: string,
    phoneNumber: string,
    rib: string
  ) => {
    const res = await addDeliveryCompanyAPI(company, email, phoneNumber, rib);
    if (res.error) onAlert(false, res.error, true);
    else {
  ...
  return (
    <div className="delivery-companies-page">
      <div className="header">
        ...
        />
        {admin?.id == 0 && (
          <div onClick={onAddCompanyButton} className="add-company-button">
            Add company
          </div>
        )}
      </div>
      ...
      <div className="list">
        {companies.map((item) => (
          <div className="company" key={item.id}>
            <DeliveryCompanyCard
              onDelete={() => onDeleteCompanyButton(item.id)}
              company={item}
              key={item.id}
            />
          </div>
