...
import { DeliveryType, IAdminFullTransaction, TransactionStatus } from "../helper/types";
...
const TransactionScreen: React.FC = () => {
  ...
  const getDataFromState = ()  : TransacionTransformedData[] => {
    const data =   transactionState.transactions ? transactionState.transactions.map((item : IAdminFullTransaction) => ({
      uuid: item.uuid,
  ...
  const [dataFiltred , setDataFiltred] = useState<TransacionTransformedData[]>([]);
  ...
  const getData = () => {
    ...
    const data : TransacionTransformedData[] = isSearch ?  defaultData.filter(it => it.uuid.toLowerCase().includes(search.toLocaleLowerCase())) : defaultData ;
    ...
    <div className="transaction-container">
      <div className="table-header">
        <div className="table-header-left">
          ...
          <div className="descr">
            Information a
            bout Buyer , Seller , and product of transaction !
          </div>
        </div>

        <div className="table-header-right">
          <div onClick={()=>setisSearch(true)} className="search-icon-container">
            <FaSearch />
          </div>
          <div className="search-bar-container">
            <input onFocus={()=>handleInputFocus()} value={search} onChange={handleInputSearchChange} placeholder="Search uuid"  />
          </div>
        </div>
      </div>
      ...
      <div className="table-container">
        ...
        >
          ...
          <Column width={80} fixed="right" align="center">
            ...
            <Cell style={{ padding: "12px" }}><FaEye className="icon-details" /></Cell>
          </Column>
        </Table>
